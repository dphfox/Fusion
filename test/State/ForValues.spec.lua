...
return function()
	...
	it("iterates on constants", function()
		...
		expect(peek(forObject)[1]).to.equal("FOO")
		expect(peek(forObject)[2]).to.equal("BAR")
		doCleanup(scope)
	end)
	...
	it("computes with constants", function()
		...
		expect(peek(forObject)[1]).to.equal("foobaz")
		expect(peek(forObject)[2]).to.equal("barbaz")
		doCleanup(scope)
	end)
	...
	it("computes with state objects", function()
		...
		expect(peek(forObject)[1]).to.equal("foofirst")
		expect(peek(forObject)[2]).to.equal("barfirst")
		suffix:set("second")
		expect(peek(forObject)[1]).to.equal("foosecond")
		expect(peek(forObject)[2]).to.equal("barsecond")
		doCleanup(scope)
	end)
	...
	it("omits values that error", function()
		...
		expect(peek(forObject)[1]).to.equal("foo")
		expect(peek(forObject)[2]).to.equal("baz")
		expect(peek(forObject)[3]).to.equal(nil)
		omitThird:set(true)
		expect(peek(forObject)[1]).to.equal("foo")
		expect(peek(forObject)[2]).to.equal(nil)
		expect(peek(forObject)[3]).to.equal(nil)
		omitThird:set(false)
		expect(peek(forObject)[1]).to.equal("foo")
		expect(peek(forObject)[2]).to.equal("baz")
